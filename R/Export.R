...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("CohortsAll.csv", package = "ACESOCovid19")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(exposureId, ".json"), package = "ACESOCovid19")
    if(cohortFileName == "") {
      definition <- ""
    } else {
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("Cohorts.csv", package = "ACESOCovid19")
  cohortsToCreate <- read.csv(pathToCsv)
  createOutcomeRow <- function(outcomeId) {
    cohortName <- as.character(cohortsToCreate$cohortName[cohortsToCreate$cohortId == outcomeId])
    cohortFileName <- system.file("cohorts", paste0(outcomeId, ".json"), package = "ACESOCovid19")
    definition <- readChar(cohortFileName, file.info(cohortFileName)$size)
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "ACESOCovid19")
...
exportMetadata <- function(outputFolder,
  ...
  database <- tibble(databaseId = databaseId,
                     ...
                     studyPackageVersion = utils::packageVersion("ACESOCovid19"),
                     isMetaAnalysis = 0)
  ...
  reference <- readRDS(file.path(outputFolder, "cmOutput", "outcomeModelReference.rds"))
  reference <- reference[reference$outcomeId %in% outcomesOfInterest, ]
  reference <- reference[!(reference$strataFile == "" & reference$studyPopFile == ""), ]
...
exportDiagnostics <- function(outputFolder,
  ...
  outcomesOfInterest <- getOutcomesOfInterest()
  reference <- reference[reference$outcomeId %in% outcomesOfInterest, ]
  reference <- reference[reference$studyPopFile!="", ]
  ...
  cluster <- ParallelLogger::makeCluster(min(1, maxCores))
  ParallelLogger::clusterRequire(cluster, "ACESOCovid19")
...
getObservationPeriodDateRange <- function(connection, cdmDatabaseSchema) {
  ...
  sql <- "SELECT MIN(observation_period_start_date) min_date FROM @cdm_database_schema.observation_period;"
  
  observationPeriodDateRange1 <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
                                                                            sql = sql,
                                                                            cdm_database_schema = cdmDatabaseSchema,
                                                                            snakeCaseToCamelCase = TRUE)
  
  sql <- "SELECT MAX(observation_period_end_date) max_date FROM @cdm_database_schema.observation_period;"
  
  observationPeriodDateRange2 <- DatabaseConnector::renderTranslateQuerySql(connection = connection,
                                                                            sql = sql,
                                                                            cdm_database_schema = cdmDatabaseSchema,
                                                                            snakeCaseToCamelCase = TRUE)
  observationPeriodDateRange <- cbind(observationPeriodDateRange1, observationPeriodDateRange2)
