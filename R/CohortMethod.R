...
runCohortMethod <- function(connectionDetails,
  ...
  cohortCounts <- read.csv(file.path(outputFolder, "CohortCountsAll.csv"))
  removeCohorts <- cohortCounts[stringr::str_detect(cohortCounts$cohortName, "Outcome") & cohortCounts$cohortSubjects<5, "cohortId"] 
  outcomesOfInterest <- outcomesOfInterest[!outcomesOfInterest%in%c(removeCohorts)]
  ...
  results <- CohortMethod::runCmAnalyses(connectionDetails = connectionDetails,
                                         ...
                                         getDbCohortMethodDataThreads = min(1, maxCores),
